...
auto
time_run(int num_iter, void (*run_game_fn)(int), int board_size) {
...
void
player_game(int board_size, agent::Agent& agent) {
    auto game = gamestate::GameStateFast::newGame(board_size, &nbr_table);
    while (not game->isOver()) {
        std::cout << "\033[2J\033[1;1H";
        utils::printBoard(game->board());
...
//              << "[µs]" << std::endl;
    ...
    //    auto bot_white = agent::RandomBot();
        auto bot_white = agent::MinMaxAgent();
        player_game(9, bot_white);
}
